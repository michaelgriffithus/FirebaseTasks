...
class SigninWithAppleCoordinator : NSObject, ASAuthorizationControllerPresentationContextProviding {
    private var onSignin: (() -> Void)? // call back to be invoked on flow success, it is passed through
        ...
        if let appleIDCredential = authorization.credential as? ASAuthorizationAppleIDCredential {
            ...
            Auth.auth().currentUser?.link(with: credential, completion:{ (authresult, error) in
                if let error = error, (error as NSError).code == AuthErrorCode.credentialAlreadyInUse.rawValue{
                    print("User is already been linked to another account");
                    if let updatedCredential = (error as NSError).userInfo[AuthErrorUserInfoUpdatedCredentialKey] as? OAuthCredential{
                        print("Signing in using updated credentials")
                        Auth.auth().signIn(with: updatedCredential, completion: {(authResult, error) in
                    ...
                    // call back, flow has finished, sign in form can be dismissed
                    if let callback = self.onSignin {
                        callback()
                    }
                }
